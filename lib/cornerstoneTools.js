...
(function($, cornerstone, cornerstoneTools) {
    ...
    function plotReferenceLine(e, eventData) {
        ...
        for (var i = 0; i < toolData.data.length; i++) {
            ...
            var handleStartCanvas = {x:0,y:data.centerPoint.y};
            var handleEndCanvas = {x:eventData.canvasContext.canvas.width,y:data.centerPoint.y};

            context.moveTo(handleStartCanvas.x, handleStartCanvas.y);
            context.lineTo(handleEndCanvas.x, handleEndCanvas.y);
			context.stroke();

            handleStartCanvas = {x:data.centerPoint.x, y:0};
            handleEndCanvas = {x:data.centerPoint.x, y:eventData.canvasContext.canvas.height};

            context.moveTo(handleStartCanvas.x, handleStartCanvas.y);
            context.lineTo(handleEndCanvas.x, handleEndCanvas.y);
            context.stroke();
            
			var text= '(x:'+(Math.round(data.centerPoint.x)).toString()+', y:'+(Math.round(data.centerPoint.y)).toString()+')';
			context.font = font;
            cornerstoneTools.drawTextBox(context, text, data.centerPoint.x, 10, color);
			context.restore(); 
        
    }
	}
    
	function reconstructImage(imagePoint, imageList, seriesIndex) {
		...
		var storedPixelsV = [];
		var sampleImg;
		for(var id=0; id<imageList.length; id++) {
		
			cornerstone.loadAndCacheImage(imageList[id]).then(function(image) {
				if (id==0){
					sampleImg=image;
				}
				var tmpPixelmatrix = image.getPixelData();
				var unitNum =Math.round(tmpPixelmatrix.length/image.columns/image.rows);
				for(var column=0; column < image.columns; column++) {
					for(var index=0; index < unitNum; index++) {
						var spIndex = ((y-1) * image.columns + column) * unitNum + index;
						
						storedPixels[pixelNum++] = tmpPixelmatrix[spIndex];
						
				}
				}
				for(var row=0; row < image.rows; row++) {
					for(var index=0; index < unitNum; index++) {
						var spIndex = (row * image.columns + x) * unitNum + index;
		...
		var minMax = getMinMax(storedPixels);

        function getPixelData() {
            return storedPixels;
        }
		
		function getPixelDataV() {
        ...
        var horizontalImg = {
            ...
            color: false,
            columnPixelSpacing: sampleImg.columnPixelSpacing,
            rowPixelSpacing: sampleImg.rowPixelSpacing*sampleImg.rows/imageList.length,
            //data: dataSet,
        ...
        if(horizontalImg.windowCenter === undefined) {
            var maxVoi = horizontalImg.maxPixelValue * horizontalImg.slope + horizontalImg.intercept;
            var minVoi = horizontalImg.minPixelValue * horizontalImg.slope + horizontalImg.intercept;
		...
		var verticalImg = {
            imageId : seriesIndex.toString()+'.'+x.toString()+'.'+y.toString()+'.'+'V',
            minPixelValue : minMax.min,
            maxPixelValue : minMax.max,
            slope: sampleImg.slope,
            intercept: sampleImg.intercept,
            ...
            windowWidth : sampleImg.windowWidth,
            render: sampleImg.render,
            getPixelData: getPixelDataV,
            rows: imageList.length,
            columns: sampleImg.rows,
            height: imageList.length,
            width: sampleImg.rows,
            color: false,
            columnPixelSpacing: sampleImg.columnPixelSpacing*sampleImg.columns/sampleImg.rows,
            rowPixelSpacing: sampleImg.rowPixelSpacing*sampleImg.rows/imageList.length,
            //data: dataSet,
            invert: sampleImg.invert,
            sizeInBytes: sampleImg.sizeInBytes
        };
		var resultImg = {h:horizontalImg,v:verticalImg};
		return resultImg;
	}
    ...
    function mouseDownCallback(e, eventData) {
        if (cornerstoneTools.isMouseButtonEnabled(eventData.which, e.data.mouseButtonMask)) {
			...
			cornerstoneTools.clearToolState(eventData.element, toolType);
			
			cornerstoneTools.addToolState(eventData.element, toolType, {
            centerPoint: e.currentPoints.canvas,
			imagePoint: e.currentPoints.image,
            });
            cornerstone.updateImage(eventData.element);
			var stackState = cornerstoneTools.getToolState(eventData.element, 'stack');
			
			var resultImg = reconstructImage(e.currentPoints.image, stackState.data[0].imageIds, stackState.data[0].seriesIndex);			
			cornerstone.displayImage(e.data.viewportH, resultImg.h);
			cornerstone.displayImage(e.data.viewportV, resultImg.v);
            return false; // false = cases jquery to preventDefault() and stopPropagation() this event
        }
		else if (cornerstoneTools.isMouseButtonEnabled(eventData.which, 4)) {
			
			var toolData = cornerstoneTools.getToolState(eventData.element, toolType);
			if (!toolData) {
            return false;
			}
			var newCenterPoint=toolData.data[0].centerPoint;
			var newImagePoint=toolData.data[0].imagePoint;
			if (e.currentPoints.canvas.x>toolData.data[0].centerPoint.x && e.currentPoints.canvas.y>toolData.data[0].centerPoint.y){
				newCenterPoint.x++;	
				newImagePoint.x++;
			}
			else if (e.currentPoints.canvas.x<toolData.data[0].centerPoint.x && e.currentPoints.canvas.y<toolData.data[0].centerPoint.y){
				newCenterPoint.x--;	
                newImagePoint.x--;				
			}
			if (e.currentPoints.canvas.y>toolData.data[0].centerPoint.y && e.currentPoints.canvas.x<toolData.data[0].centerPoint.x){
				newCenterPoint.y++;	
                newImagePoint.y++;				
			}
			else if (e.currentPoints.canvas.y<toolData.data[0].centerPoint.y && e.currentPoints.canvas.x>toolData.data[0].centerPoint.x){
				newCenterPoint.y--;		
                newImagePoint.y--;				
			}
			...
			var resultImg = reconstructImage(newImagePoint, stackState.data[0].imageIds, stackState.data[0].seriesIndex);			
			cornerstone.displayImage(e.data.viewportH, resultImg.h);
			cornerstone.displayImage(e.data.viewportV, resultImg.v);
            return false; // false = cases jquery to preventDefault() and stopPropagation() this event
    ...
    function enable(element, mouseButtonMask, targetElementH, targetElementV) {
        var eventData = {
            mouseButtonMask: mouseButtonMask,
			viewportH:targetElementH,
			viewportV:targetElementV,
        };
		...
		$(element).on('CornerstoneImageRendered', plotReferenceLine);	
    ...
    function disable(element) {
        $(element).off('CornerstoneToolsMouseDown', mouseDownCallback);
		$(element).off('CornerstoneImageRendered', plotReferenceLine);	
    }

    // module/private exports
    cornerstoneTools.mpr = {
        activate: enable,
        deactivate: disable,
        enable: enable,
        disable: disable
